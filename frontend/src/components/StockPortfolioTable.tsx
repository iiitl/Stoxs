...
function StockPortfolioTable() {
  ...
  const handlePutRequest = async (stock : Stock) => {
    fetch(`${import.meta.env.URL}/api/stocks/${stock.id}`, {
      method: 'PUT',
      headers: {
  ...
  const getStocks = async () => {
    ...
    , 2000)
    const response = await fetch(`${import.meta.env.URL}/api/stocks`)
  ...
  const deleteStock = async (id: string) => {
    ...
    await fetch(`${import.meta.env.URL}/api/stocks/${id}`, {
      method: 'DELETE'
    })
  }
  ...
  const getStockPrice = async (ticker: string) => {
    setLoading(true)
    const response = await fetch(`${import.meta.env.URL}/api/stocks/${ticker}/price`)
    const data = await response.json()
    setLoading(false)
    return data.c
  }
  ...
  const handleSave = async (id: string) => {
    ...
    const rtn = await fetch(`${import.meta.env.URL}/api/stocks/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
